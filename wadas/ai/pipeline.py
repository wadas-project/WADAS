...
####
from wadas.ai.schedulers_definitivo import (Monitoring_Scheduler, Actuator_Scheduler, Optimum_Scheduler)

from wadas.ai.models_custom import (
    Classifier,
    OVMegaDetectorV5,
...
USECASE_TO_SCHEDULER={
    ...
    "ACTUATOR": Actuator_Scheduler,
    "CRITICAL": Optimum_Scheduler, 
} 
...
class DetectionPipeline:
    ...
    def __init__(
        ...
        if self.distributed_inference:
            ray.init()
            ####
            if not (scheduling_cls:= USECASE_TO_SCHEDULER.get(use_case)):
                return ValueError("Invalid use case:"+ use_case)
            else:
                self.scheduler=scheduling_cls()
                best_nodes= self.scheduler.best_nodes_det_class()
                self.best_detection_node = best_nodes[0]
                self.best_classification_node = best_nodes[1]
                print(f"NODO PER DETECTION {self.best_detection_node}----------------------------NODO PER CLASSIFICATION {self.best_classification_node}")
    ...
    def initialize_model(self, cls, *args, **kwargs):
        ...
        if self.distributed_inference:
            ...
            #controllo se sto inizializzando un modello di detector o classification
            if cls in NAME_TO_DETECTOR.values():
                print("CREAZIONE MODELLO DETECTION IN REMOTO!!!")
                best_node=self.best_detection_node['id_node'] #per detection prendo il primo nodo
            else:
                print("CREAZIONE MODELLO CLASSIFICATION IN REMOTO!!!")
                best_node=self.best_classification_node["id_node"] #per classification prendo il secondo nodo

            remote_cls = ray.remote(cls).options(
                scheduling_strategy=NodeAffinitySchedulingStrategy(
                    node_id = best_node,
                    soft = False, #PER NON FAR FALLIRE IL TASK SE IL NODO CANDIDATO MUORE TRUE
                )
                #resources={"NPU":0,},
                #num_cpus=3,
                #num_gpus=0
            ).remote(*args, **kwargs)
            print(f"!!!!!!ISTANZA DEL MODELLO CREATA IN REMOTO SUL NODO {best_node}!!!!!!!")
    ...
    def run_model(self, fn, *args, **kwargs):
        ...
        #if self.distributed_inference:
           # best_nodes=self.scheduler.best_nodes_det_class()
           # classe=fn.__call__.__class__
           # if classe.__name__== 'Classifier':
     ...
     #   if self.distributed_inference:
        #    func=fn._remote(args=[*args], kwargs={**kwargs}, num_cpus=risorse["CPU"], num_gpus=risorse["GPU"], resources={"NPU": risorse["NPU"]})
     #       func = fn.options(
     #           num_cpus=risorse["CPU"],
     #           num_gpus=risorse["GPU"],
     #           resources={"NPU": risorse["NPU"]} ,
     #       ).remote
     #   else:
     #       func=fn
